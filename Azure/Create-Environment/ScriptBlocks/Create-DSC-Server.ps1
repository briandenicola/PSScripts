...
Set-Variable -Name dsc_database_path_source -Value (Join-Path -Path $pshome -ChildPath "modulespsdesiredstateconfiguration\pullserver\devices.mdb") -Option Constant
Set-Variable -Name dsc_database_path_destination -Value (Join-Path -Path $env:programfiles -ChildPath "WindowsPowerShell\DscService") -Option Constant
...
Copy-Item -Path $dsc_database_path_source -Destination $dsc_database_path_destination  -Verbose
$cfg = [xml] ( Get-Content (Join-Path $path "web.config") ) foreach( $setting in $settings )  {    $add = $cfg.CreateNode( [System.Xml.XmlNodeType]::Element, "add", $null)
