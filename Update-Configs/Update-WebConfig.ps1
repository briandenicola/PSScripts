...
. (Join-Path $PWD.Path "Modules\File_Functions.ps1")

$cfg = [xml] (Get-Content -Path $config_updates)
$global:log_file = Join-Path -Path $PWD.Path -ChildPath ("Logs\Config-File-Update-{0}.log" -f $(Get-Date).ToString("yyyyMMdd"))
